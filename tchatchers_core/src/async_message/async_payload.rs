...
#[derive(Clone, derive_more::Display, Debug, derive_more::Error)]
...
impl Into<[(String, String); 2]> for AsyncPayload {
    fn into(self) -> [(String, String); 2] {
        [
            ("val".into(), serde_json::to_string(&self.entity).unwrap()),
...
impl AsyncPayload {
    ...
    pub(crate) fn read_events(queue_name: &str, conn: &mut redis::Connection) -> Option<Vec<Self>> {
        let stream_events: Option<StreamReadReply> = conn
            .xread_options(&[queue_name], &["$"], &DEFAULT_EVENT_OPTIONS)
            .unwrap();
            ...
            for keys in stream_events.keys {
                for id in keys.ids {
                    ...
                    match Self::try_from((queue_name, event_id.as_str(), map)) {
                        ...
                        Err(e) => error!("[{queue_name}] An error happened while parsing the value of the event #{event_id} : \n{e}"),
                    }
                }
            }
            debug!(
