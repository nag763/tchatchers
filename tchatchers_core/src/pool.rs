...
pub async fn get_pg_pool() -> Result<PgPool, sqlx::Error> {
    let connect_options = sqlx::postgres::PgConnectOptions::new_without_pgpass()
...
pub async fn get_session_pool() -> Result<redis::aio::MultiplexedConnection, redis::RedisError> {
    ...
    let client = redis::Client::open(format!("redis://{redis_host}:{redis_port}/1"))?;
    client.get_multiplexed_async_connection().await
}

#[cfg(any(feature = "back", feature = "async", feature = "cli"))]
pub async fn get_async_pool() -> Result<redis::aio::MultiplexedConnection, redis::RedisError> {
    let redis_host = std::env::var("REDIS_HOST").expect("No redis host defined in .env");
    let redis_port = std::env::var("REDIS_PORT").expect("No redis port defined in .env");
    let client = redis::Client::open(format!("redis://{redis_host}:{redis_port}/2"))?;
    client.get_multiplexed_async_connection().await
}
