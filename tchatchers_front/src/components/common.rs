...
pub fn waiting_for_response() -> Html {
    html! {
        <p class="flex justify-center">{"Waiting for server reply"}
          <svg class="inline ml-2 w-6 h-6 text-gray-200 animate-spin dark:text-gray-600 fill-gray-800" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
...
pub fn form_button(props: &FormButtonProperties) -> Html {
    html! {
      <div class="flex items-center">
        <div class="w-2/3"></div>
        <div class="w-1/3">
          <button class="shadow bg-zinc-800 enabled:hover:bg-zinc-900 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="submit">
          {&props.label}
          </button>
        </div>
      </div>
