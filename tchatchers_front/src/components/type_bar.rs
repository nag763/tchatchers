        ...
        html! {
            <>
                ...
                <div class="col-span-6 my-6">
                      <input class="shadow appearance-none border rounded-xl px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-zinc-500 w-full h-10 invalid:border-red-500 disabled:bg-gray-100 focus:invalid:border-red-500 bg-gray-200" id="username" type="text" placeholder={placeholder_input} minlength="2" maxlength="127" ref={self.input_ref.clone()} disabled={!self.can_post} onkeydown={ctx.link().callback(|e : yew::KeyboardEvent | { if e.code() == "Enter" { Msg::SubmitForm } else { Msg::IgnoreEvent }})}/>
                      <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700" hidden={self.can_post}>
                        <div class="bg-gradient-to-r from-zinc-600 to-zinc-700 h-2.5 rounded-full" style={format!("width: {}%", self.progress_percentage*100/TIMEOUT)}>
                        </div>
                    </div>
                </div>
        </>
